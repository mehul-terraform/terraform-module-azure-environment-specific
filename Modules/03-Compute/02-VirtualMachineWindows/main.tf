...
resource "azurerm_windows_virtual_machine" "virtual_machine" {
  ...
  patch_mode            = lookup(each.value, "patch_mode", "AutomaticByPlatform")
  ...
  source_image_reference {
    publisher = lookup(each.value, "image_publisher", "MicrosoftWindowsServer")
    offer     = lookup(each.value, "image_offer", "WindowsServer")
    sku       = lookup(each.value, "image_sku", "2022-datacenter")
